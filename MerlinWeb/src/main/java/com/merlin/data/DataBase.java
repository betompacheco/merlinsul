    ...
    public static PreparedStatement getStatementFiltered(List filtros, Connection con, String qry) throws SQLException {
        ...
        if (filtros != null) {
            for (int i = 0; i < filtros.size(); i++) {
                /*
                 * Filter filtro = (Filter) filtros.get(i); Field campo = filtro.getField(); if (filtro.getValues().size()>0) { Object valor=filtro.getValues().get(0); if (campo.getType()==Field.TEXT){ if (valor!=null && valor.toString().trim().length()>0){ where+= (where.length()>0?" AND ":" ") +
                 * campo.getName() + " like ? "; parametros.add(valor.toString() + "%"); } } }
                 */
            }
        }
